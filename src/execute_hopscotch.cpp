...
// Join algorithm using an unordered_map build (with safe normalization)
struct JoinAlgorithm {
    ...
    auto run() {
        ...
        // This avoids ill-formed expressions for incompatible Key/T pairs.
            ...
            for (auto& right_record : right) {
                std::visit(
                    [&](const auto& key) {
                        ...
                        if constexpr (!std::is_same_v<KeyT, std::monostate>) {
                            if (auto opt = try_normalize.template operator()<KeyT>(key)) {
                                if (auto it = hash_table.find(*opt); it != hash_table.end()) {
