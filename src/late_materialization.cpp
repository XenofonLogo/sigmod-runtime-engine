...
static bool column_has_nulls(const Column& column) {
    if (column.type != DataType::INT32) return true;
    if (column.pages.empty()) return false;
    
    // Quick check: only check first page
    // If first page has no nulls, assume column has no nulls (heuristic)
    auto* page = column.pages[0]->data;
    ...
    for (size_t i = 0; i < num_bytes; ++i) {
        if (i == num_bytes - 1) {
            size_t remaining = num_rows - (i * 8);
            uint8_t mask = (1u << remaining) - 1;
            if ((bitmap[i] & mask) != mask) return true;
        } else {
            if (bitmap[i] != 0xFF) return true;
        }
    }
    
    // First page has no nulls - assume rest don't either
    return false;
}
