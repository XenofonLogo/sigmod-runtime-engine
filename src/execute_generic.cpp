...
namespace Contest
{
    ...
    static RowStore columnbuffer_to_rowstore(const ColumnBuffer &buf)
    {
        ...
        // Every row has num_cols columns
        for (auto &r : rows)
        {
    ...
    // We use columnar.cpp for lm scan but we convert into row-store
                                          ...
                                          const RowStore &right)
    {
        ...
        // Helper:creates row output according to output_attrs
    ...
    // Recursive executor with RowStore
    ...
    ColumnarTable execute(const Plan &plan, [[maybe_unused]] void *context)
    {
        ...
        return finalize_columnbuffer_to_columnar(plan, buf, attrs); // ColumnBuffer -> ColumnarTable (final materialization for VARCHAR)
    }
